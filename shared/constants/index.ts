export const APP_NAME = 'Dénonciation';
export const APP_VERSION = '1.0.0';
export const APP_DESCRIPTION = 'Plateforme de signalement d\'abus et d\'injustices';

export const API_BASE_URL = process.env.API_URL || 'http://localhost:5000/api';
export const SOCKET_URL = process.env.SOCKET_URL || 'http://localhost:5000';

export const DEFAULT_AVATAR = 'https://ui-avatars.com/api/?background=EF4444&color=fff&name=';
export const DEFAULT_THUMBNAIL = 'https://via.placeholder.com/640x360/1F2937/FFFFFF?text=LIVE';

export const CATEGORIES = [
  { name: 'Corruption', icon: '💰', color: '#e74c3c' },
  { name: 'Violence policière', icon: '👮', color: '#c0392b' },
  { name: 'Discrimination', icon: '⚖️', color: '#f39c12' },
  { name: 'Violences sexuelles', icon: '🚫', color: '#9b59b6' },
  { name: 'Abus de pouvoir', icon: '👑', color: '#e67e22' },
  { name: 'Fraude électorale', icon: '🗳️', color: '#2c3e50' },
  { name: 'Atteinte à l\'environnement', icon: '🌳', color: '#27ae60' },
  { name: 'Non-respect des droits humains', icon: '✊', color: '#3498db' },
  { name: 'Autre', icon: '📌', color: '#95a5a6' },
];

export const REPORT_STATUS = {
  pending: { label: 'En attente', color: '#F59E0B', bgColor: '#FEF3C7' },
  approved: { label: 'Approuvé', color: '#10B981', bgColor: '#D1FAE5' },
  rejected: { label: 'Rejeté', color: '#EF4444', bgColor: '#FEE2E2' },
  archived: { label: 'Archivé', color: '#6B7280', bgColor: '#F3F4F6' },
};

export const STREAM_STATUS = {
  active: { label: 'En direct', color: '#EF4444' },
  ended: { label: 'Terminé', color: '#6B7280' },
  cancelled: { label: 'Annulé', color: '#F59E0B' },
};

export const NOTIFICATION_TYPES = {
  welcome: 'Bienvenue',
  new_report: 'Nouveau signalement',
  new_comment: 'Nouveau commentaire',
  new_like: 'Nouveau like',
  new_witness: 'Nouveau témoignage',
  new_live: 'Nouveau live',
  report_status: 'Statut du signalement',
  system: 'Système',
};

export const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB
export const ALLOWED_IMAGE_TYPES = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'];
export const ALLOWED_VIDEO_TYPES = ['video/mp4', 'video/mpeg', 'video/quicktime'];
export const ALLOWED_DOCUMENT_TYPES = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'];

export const PAGINATION = {
  DEFAULT_PAGE: 1,
  DEFAULT_LIMIT: 20,
  MAX_LIMIT: 100,
};

export const RATE_LIMITS = {
  login: 10,
  register: 5,
  createReport: 10,
  createComment: 30,
};