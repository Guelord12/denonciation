...
const Notifications = () => {
  ...
  const handleClick = (notification) => {
    ...
    // Sinon, on reste sur la page des notifications
  };
      ...
      <div className="container">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          ...
          {notifications.length > 0 && (
            <button onClick={markAllAsRead}>{t('notifications.markAllRead')}</button>
          )}
        </div>
        {notifications.length > 0 ? (
          <div>
            {notifications.map(n => (
              <div
                ...
                style={{
                  padding: '1rem',
                  marginBottom: '0.5rem',
                  background: n.is_read ? '#f5f5f5' : '#fff3e0',
                  borderRadius: '8px',
                  cursor: 'pointer',
                  borderLeft: `4px solid ${n.is_read ? '#ccc' : '#e63946'}`,
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <small style={{ color: '#666' }}>{formatDate(n.created_at)}</small>
                  </div>
                  {!n.is_read && (
                    <span style={{ background: '#e63946', color: 'white', padding: '0.25rem 0.5rem', borderRadius: '20px', fontSize: '0.75rem' }}>
